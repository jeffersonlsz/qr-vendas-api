...
class MetricasParceiro(BaseModel):
    ...
    total_solicitacoes: int = Field(default=0, description="Total de solicitações geradas")
    solicitacoes_novas: int = Field(default=0, description="Solicitações novas")
    solicitacoes_em_atendimento: int = Field(default=0, description="Solicitações em atendimento")
    solicitacoes_convertidas: int = Field(default=0, description="Solicitações convertidas")
    solicitacoes_perdidas: int = Field(default=0, description="Solicitações perdidas")
...
class DashboardResumo(BaseModel):
    ...
    ultimas_solicitacoes: list[dict] = Field(default_factory=list)
...
class DashboardGeral(BaseModel):
    ...
    total_solicitacoes: int = Field(default=0, description="Total de solicitações")
    total_vendas: int = Field(default=0, description="Total sales")
    valor_total_vendas: float = Field(default=0.0, description="Total sales value")
